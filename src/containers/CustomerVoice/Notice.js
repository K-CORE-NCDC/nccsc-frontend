  ...
  const fetchUsers = async (page, method) => {
    ...
    if (method === 'GET') {
      response = await axios.get(
        config.auth + `notice-api/?page=${page}&per_page=${perPage}&delay=1`
      );
    } else {
      response = await axios.post(
        config.auth + `notice-api/?page=${page}&per_page=${perPage}&delay=1&input`,
        {
          type: selectInput,
              ...
              <div className="flex float-right">
                <div className="flex-none w-40 h-14">
                  <select
                    ...
                    name="cars"
                    id="cars"
                    className="border border-slate-400 rounded pt-1 pb-1"
                  >
                    <option className="text-xl" value="title">
                      {title}
                    </option>
                    <option className="text-xl" value="content">
                      {content}
                    </option>
                    <option className="text-xl" value="writer">
                      {writer}
                    </option>
                  </select>
                </div>
...
export default function Notice() {
  ...
  return <>{slug ? <NoticeDetail slug_id={slug} /> : <NoticeList />}</>;
}
