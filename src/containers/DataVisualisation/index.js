...
export default function DataVisualization() {
  ...
  useEffect(() => {
    ...
    l.forEach(element => {
      let classes = 'lg:px-4 sm:px-2 xs:px-2 py-2 xs:text-sm sm:text-xl lg:text-2xl font-semibold rounded-t opacity-50 '
      if (tab === element) {
          ...
          <div className={toggle?"grid grid-cols-4":"grid "}>
            ...
            <div className={toggle?"xs:absolute lg:relative col-start-2 col-span-3 overflow-auto":""}>
              <div className="grid grid-cols-3 gap-1 p-5 bg-white">
                <div className={`col-span-3 lg:hidden md:hidden ${tab === "volcano"?"xs:hidden":""}`}>
                ...
                <div className='flex xs:col-span-3 lg:col-span-3 sm:col-span-3 sm:gap-0 xs:gap-0 lg:gap-6'>
                  ...
                  <div className="inline-flex lg:w-2/5 xs:w-60">
                    <select id='gene_type' value={state['type']} onChange={e => selectGene(e)} className='btn_input_height lg:w-full xs:w-56 p-3 border bg-white focus:outline-none border-blue-300 focus:ring focus:border-blue-300 xs:h-14 lg:h-16 xs:text-sm lg:text-xl'>
                      <option className="xs:text-sm lg:text-xl" value="user-defined">User-Defined List</option>
                      <option className="xs:text-sm lg:text-xl" value="major-genes">Cancer major genes (28 genes)</option>
                      <option className="xs:text-sm lg:text-xl" value="brst-major-genes">Breast cancer major genes (20 genes)</option>
                      <option className="xs:text-sm lg:text-xl" value="hrd-genes">HRD genes (15 genes)</option>
                      <option className="xs:text-sm lg:text-xl" value="hrd-asso-brst">HRD association breast (26 genes)</option>
                      <option className="xs:text-sm lg:text-xl" value="tcell-exha-genes">Tcell exhausted genes (8 genes)</option>
                      <option className="xs:text-sm lg:text-xl" value="cdc-brst-genes">CDC Phenopedia breast cancer associated genes (18 genes)</option>
                      <option className="xs:text-sm lg:text-xl" value="cell-cycle-ctrl">General: Cell Cycle Control (34 genes)</option>
                      <option className="xs:text-sm lg:text-xl" value="p53-signal">General: p53 signaling (6 genes)</option>
                      <option className="xs:text-sm lg:text-xl" value="notch-signal">General: Notch signaling (55 genes)</option>
                      <option className="xs:text-sm lg:text-xl" value="dna-damage-resp">General: DNA Damage Response (12 genes)</option>
                      <option className="xs:text-sm lg:text-xl" value="other-grow-prol-signal">General: Other growth/proliferation signaling (11 genes)</option>
                      <option className="xs:text-sm lg:text-xl" value="survival-cell-signal">General: Survival/cell death regulation signaling (23 genes)</option>
                      <option className="xs:text-sm lg:text-xl" value="telo-mere-main">General: Telomere maintenance  (2 genes)</option>
                      <option className="xs:text-sm lg:text-xl" value="rtk-signal-family">General: RTK signaling family (16 genes)</option>
                      <option className="xs:text-sm lg:text-xl" value="pi3k-akt-mtor-signal">General: PI3K-AKT-mTOR signaling (17 genes)</option>
                      <option className="xs:text-sm lg:text-xl" value="ras-raf-signal">General: Ras-Raf-MEK-Erk/JNK signaling (26 genes)</option>
                      <option className="xs:text-sm lg:text-xl" value="regu-ribo-cell">General: Regulation of ribosomal protein synthesis and cell growth  (9 genes)</option>
                      <option className="xs:text-sm lg:text-xl" value="angi-gene">General: Angiogenesis (6 genes)</option>
                      <option className="xs:text-sm lg:text-xl" value="fola-trans">General: Folate transport (5 genes)</option>
                      <option className="xs:text-sm lg:text-xl" value="inva-meta">General: Invasion and metastasis (27 genes)</option>
                      <option className="xs:text-sm lg:text-xl" value="tgf-beta-path">General: TGF-β Pathway (43 genes)</option>
                    </select>
                  </div>
                  <div className="inline-flex lg:w-2/5 sm:w-13 xs:w-60">
                    <input type="text" id='genes' className='btn_input_height lg:w-full sm:w-13 xs:w-56 p-3 xs:text-sm lg:text-xl border focus:outline-none border-blue-300 focus:ring focus:border-blue-300 xs:h-14 lg:h-16' name='genes' />
                  </div>
                  <div className="inline-flex lg:w-2/12 sm:w-1/5">
                    <button className="btn_input_height bg-main-blue hover:bg-main-blue mb-3 xs:text-sm lg:text-xl lg:w-full text-white ml-2 font-bold py-2 px-4 border border-blue-700 rounded xs:h-14 lg:h-16" onClick={submitFilter}><FormattedMessage  id = "Filter" defaultMessage='Filter'/></button>
                  </div>
                </div>
              </div>
              <div className='gap-6'>
                <section>
                  <nav id="second_header" className=" px-8 pt-2 shadow-md" >
                    <ul id="tabs" className="inline-flex justify-center w-full lg:px-1 sm:px-0 pt-2" onClick={e => toggleTab(e)}>
                      {menuItems}
                    </ul>
                  </nav>
                </section>
