...
import { getFusionExons, getFusionInformation } from '../../actions/api_actions';
      ...
      for (const key in fusionPlotJson['exons']) {
        ...
        for (let index = 0; index < r.length; index++) {
          ...
          if (id === 'leftGene' && leftSecondRow) {
            htmlExons1.push(
              <div
                title={'exon -' + (index + 1) + ' ' + element.startCodon}
                key={index}
                style={{
                  width: w + 'px',
          ...
          if (
            exon_pos === element.endCodon ||
            exon_pos === element.startCodon ||
            (exon_pos >= element.startCodon && exon_pos <= element.endCodon)
          ) {
            htmlExons.push(
              <div
                title={'exon -' + (index + 1) + ' ' + element.startCodon}
              ...
              >
                <div
                  style={{ backgroundColor: element.color, height: '20px', marginTop: '60px' }}
                ></div>
              </div>
            );
            if (left_w) {
        ...
        h.push(
          <div key={key} className="ChromosomeFusionPlotDivision">
            <h3 style={{ color: fusionJson['exons'][key][0].color }}>
              {key} -- {fusionPlotJson['pos'][key]}
            </h3>

            <div
              id={'row_' + i}
              className={
                'grid_row Flex JustifyCenter AlignItemsFlexEnd  MarginTop10 Relative ' +
                id +
                ' ' +
                direction
              }
              style={{
                color: fusionJson['exons'][key][0].color,
                height: '100px',
                borderBottom: '1px solid ' + fusionJson['exons'][key][0].color,
                borderColor: fusionJson['exons'][key][0].color
              }}
            >
              {htmlExons}
              <div
            ...
            <div
              className={
                'grid_row Flex  AlignItemsFlexEnd  MarginTop10 Relative ' +
                id +
                ' ' +
                name +
                ' ' +
  ...
  useEffect(() => {
    if (renderPlot && html.length > 0) {
      ...
      if (row !== null && rowLine !== null) {
        ...
        document.getElementById('leftGene1').style.left =
          rowLine.offsetWidth + rowLine.offsetLeft + 'px';
      }
      if (rightrowLine !== null) {
        document.getElementById('rightGene1').style.width =
          rightrowLine.offsetLeft + rightrowLine.offsetWidth + 'px';
            ...
            <>
              ...
              <div className="FusionChromosome">
                <div className="text-left">
                  ...
                  <select
                    onChange={(e) => transcriptChange(e, 'left')}
                ...
                <div className="text-left">
                  ...
                  <select
                    onChange={(e) => transcriptChange(e, 'right')}
                    className="w-full lg:p-4 xs:p-2 border xs:text-sm lg:text-lg focus:outline-none border-b-color focus:ring focus:border-b-color active:border-b-color mt-3"
                  >
