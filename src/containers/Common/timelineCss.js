...
const TimeLineChart = ({ yearKey, valueKey, data }) => {
    ...
    return (
        <div >
            {showGraph ? (<div className="wraper-timeline-graph" >
                <ul className="container-timeline-graph" >
                    {yearsArray.map((element, index) => (
                        <li key={`${element}-${index}`} className={`${yearsObject[element].length > 0 ? 'link-timeline-graph' : 'line-timeline-graph'} active-timeline-graph tooltip-timeline-graph`} >
                            <div>{index + 2000}</div>
