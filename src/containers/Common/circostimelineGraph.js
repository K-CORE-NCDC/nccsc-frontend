...
const GraphsModal = ({ closeShowTimelineTables, circosTimelieTableData }) => {
      ...
      if (Object.keys(circosTimelieTableData).length > 0) {
        ...
        if (ki67.length > 0) {
          ...
          for (let i = 0; i < ki67.length; i++) {
            ...
            let tooltip =
              '<div>KI67: ' + ki67[i]['ki67_score'] + '<hr/>' + ki67[i]['imnl_acpt_ymd'] + '</div>';
            if (ki67.length === 1) {
              let year = new Date(d).getFullYear();
              let month = new Date(d).getMonth();
              let day = new Date(d).getDate();
        ...
        if (ca15.length > 0) {
          ...
          for (let i = 0; i < ca15.length; i++) {
            ...
            let tooltip =
              '<div>exam_ymd: ' + ca15[i]['exam_ymd'] + '<hr/>' + ca15[i]['exam_val'] + '</div>';
            if (ca15.length === 1) {
              let year = new Date(d).getFullYear();
              let month = new Date(d).getMonth();
              let day = new Date(d).getDate();
        ...
        if (cea.length > 0) {
          ...
          for (let i = 0; i < cea.length; i++) {
            ...
            let tooltip =
              '<div>exam_ymd: ' + cea[i]['exam_ymd'] + '<hr/>' + cea[i]['exam_val'] + '</div>';
            if (cea.length === 1) {
              let year = new Date(d).getFullYear();
              let month = new Date(d).getMonth();
              let day = new Date(d).getDate();
        ...
        if (her2.length > 0) {
          ...
          for (let i = 0; i < her2.length; i++) {
            ...
            let tooltip =
              '<div>exam_ymd: ' + her2[i]['exam_ymd'] + '<hr/>' + her2[i]['exam_val'] + '</div>';
            if (her2.length === 1) {
              let year = new Date(d).getFullYear();
              let month = new Date(d).getMonth();
              let day = new Date(d).getDate();
  ...
  let activeNavClass =
    'py-4 px-6 block hover:text-slate-50 focus:outline-none font-medium text-white  bg-main-blue ';
  return (
        ...
        <div className="overflow-y-scroll  oncoimages_height">
          <div className="p-6">
            <nav className="flex flex-col sm:flex-row">
              <button
                onClick={() => setTab('bmi')}
                className={tab === 'bmi' ? activeNavClass : navClass}
              >
                BMI
              </button>
              <button
                onClick={() => setTab('ki67')}
                className={tab === 'ki67' ? activeNavClass : navClass}
              >
                KI-67 (%)
              </button>
              <button
                onClick={() => setTab('ca15')}
                className={tab === 'ca15' ? activeNavClass : navClass}
              >
                CA15-3
              </button>
              <button
                onClick={() => setTab('cea')}
                className={tab === 'cea' ? activeNavClass : navClass}
              >
                CEA
              </button>
              <button
                onClick={() => setTab('her2')}
                className={tab === 'her2' ? activeNavClass : navClass}
              >
                HER2
              </button>
            </nav>
            <div className="grid tab-content  w-full col-span-4" id="tabs-tabContent">
              <div
                className={
                  `py-6 tab-pane fade flex-inline` + (tab === 'bmi' ? 'show active' : ' hidden ')
                }
                id="tabs-bmi"
              >
                <h3>BMI Timeline</h3>
              ...
              <div
                className={
                  'py-6 tab-pane fade flex-inline ' + (tab === 'ki67' ? 'show active' : ' hidden ')
                }
                id="tabs-ki67"
              >
              ...
              <div
                className={
                  'py-6 tab-pane fade flex-inline ' + (tab === 'ca15' ? 'show active' : ' hidden ')
                }
                id="tabs-ca15"
              >
              ...
              <div
                className={
                  'py-6 tab-pane fade flex-inline ' + (tab === 'cea' ? 'show active' : ' hidden ')
                }
                id="tabs-cea"
              >
                <h3>CEA Timeline</h3>
              ...
              <div
                className={
                  'py-6 tab-pane fade flex-inline ' + (tab === 'her2' ? 'show active' : ' hidden ')
                }
                id="tabs-her2"
              >
                <h3>HER2 Timeline</h3>
