  ...
  return (
    <>
      {isDataFound ? (
        <div>
          <div
            className="grid max-auto grid-cols-5 gap-3 p-6 overflow-y-scroll"
            style={{ height: '50vh' }}
          >
            {activeDisplayImagesContent.map((e) => {
              return (
                <a key={e} target="_blank" rel="noreferrer" href={config['auth'] + e}>
          ...
          <div className="bg-white px-4 py-3 flex items-center justify-between border-t border-gray-200 sm:px-6">
            <div className="hidden w-full m-1 p-1 sm:flex sm:items-center sm:justify-between">
              <div>
                {currentActivePageNumber === totalPages ? (
                  <p className="text-md text-gray-700">
                    Showing{' '}
                    <span className="font-medium">
                      {totalImagesCount - activeDisplayImagesContent.length}
                    </span>{' '}
                    to <span className="font-medium">{totalImagesCount}</span> of{' '}
                    <span className="font-medium">{totalImagesCount}</span> results
                  </p>
                ) : (
                  <p className="text-md text-gray-700">
                    Showing{' '}
                    <span className="font-medium">{(currentActivePageNumber - 1) * 20}</span> to{' '}
                    <span className="font-medium">{currentActivePageNumber * 20}</span> of{' '}
                    <span className="font-medium">{totalImagesCount}</span> results
                  </p>
                )}
              </div>
              <div>
                <nav
                  className="w-full inline-flex rounded-md shadow-sm -space-x-px"
                ...
                >
                  ...
                  {pagenationPageNos.map((element, index) => (
                    <button
                      ...
                      value={element}
                      className={
                        element === currentActivePageNumber ? activePageCss : nonActiveClassCss
                      }
                    >
                      {element}
                    </button>
      ...
      <div className="relative w-1/2 h-1/2 my-10 mx-auto">
        ...
        <div className="border-0 rounded-lg shadow-lg relative flex flex-col w-full bg-white outline-none focus:outline-none">
          ...
          <div className="flex items-start justify-between p-5 border-b border-solid border-blueGray-200 rounded-t">
            <h3 className="lg:text-3xl sm:text-xl font-semibold">Pathological Images</h3>
