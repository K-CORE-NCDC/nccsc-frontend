  ...
  function volcanoPlot() {
    ...
    function chart(selection) {
      ...
      selection.each(function (data) {
        ...
        xScale.range([0, innerWidth]).domain([-10, 10]).nice();
        ...
        yScale.range([0, innerHeight]).domain([max, 0]).nice(); // adds "padding" so the domain extent is exactly the min and max values
        var zoom = d3
          .zoom()
          .scaleExtent([1, 20])
          .translateExtent([
        ...
        gY.append('text')
          ...
          .attr(
            'transform',
            'translate(' + (0 - margin.left / 1.25) + ',' + height / 2 + ') rotate(-90)'
          )
