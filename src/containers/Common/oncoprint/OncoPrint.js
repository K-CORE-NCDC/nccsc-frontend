    ...
    getData() {
        ...
        Object.keys(events).forEach((key) => {
            ...
            const text_arr = aggr.events.map(
                (event) => `${event.sample}<br>${getDisplayName(event, this.props.SupportedEvents)}`
            );
            ...
            data.push({
                ...
                hoverinfo: 'text',
                marker: {
                    color: getColor(aggr.events[0], colorscale, this.props.SupportedEvents)
                },
                name: getDisplayName(aggr.events[0], this.props.SupportedEvents),
                text: text_arr,
                orientation: 'h',
                type: 'bar',
                width,
