            ...
            <div className="relative w-auto my-6 mx-auto max-w-3xl">
              ...
              <div className="border-0 rounded-lg shadow-lg relative flex flex-col w-full bg-white outline-none focus:outline-none">
                ...
                <div className="flex items-start justify-between p-5 border-b border-solid border-blueGray-200 rounded-t">
                  <h3 className="text-3xl font-semibold">Error in the uploaded file</h3>
                  <button
                ...
                <div className="relative p-6 flex-auto">
                  <div className="my-4 text-2xl leading-relaxed">{body}</div>
                </div>
                {/*footer*/}
...
export default function FileUpload({ updateComponentNumber }) {
    ...
    allTableTabs.forEach((element) => {
      ...
      tableNavTabsTemp.push(
        <li key={element} className={css}>
          <button
            value={element}
            onClick={() => changeErrorDataTable(element)}
      ...
      firstInput.push(
        <div key={key} className="grid grid-cols-12 gap-6 ">
          ...
          <div className="p-5 col-span-2 flex">
            ...
            {loader[selectedFileSampleType[key]] === 'success' && (
              <section className="">
            ...
            {loader[selectedFileSampleType[key]] === 'loader' && (
              <p className="mt-5 text-center">
                <FormattedMessage
                  id="WaitMessage"
                  defaultMessage="It takes 1 or 2 minutes to process data"
                />{' '}
              </p>
            )}
            {loader[selectedFileSampleType[key]] === 'failed' && (
  ...
  const addNewHTML = () => {
    ...
    if (newElementId < 9) {
      ...
      let availableTypes = Object.keys(dropdownOptions).filter(
        (step) => !checkedFileTypes.includes(step)
      );
          ...
          <div className="grid grid-cols-6 mt-10">
            <div className=" col-start-2 col-span-4 bg-white  space-y-3 px-6 py-10 rounded-3xl shadow-lg border ">
              <div className="relative w-full col-span-12 ">
                <div className="flex flex-row-reverse ">
                  <button
                    ...
                    className="has-tooltip bg-red-500 hover:bg-red-700 text-white text-center py-2 px-4 rounded-full h-14 w-14 inline-flex items-center"
                  >
                    <InforIcon />
                  </button>
                </div>
                <div className="flex">
                  <div className="text-base sm:text-sm md:text-md lg:text-base xl:text-2xl  2xl:text-md">
                  ...
                  <div className="mb-4">
                    <input
                      ...
                      value={projectName}
                      className={` ${
                        borderRed ? 'border-red-400' : ''
                      } ml-10 shadow appearance-none border rounded w-96 h-12 py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline`}
                      required={true}
                      id="project"
                      type="text"
                ...
                <h2 className="text-base sm:text-sm md:text-md lg:text-base xl:text-2xl  2xl:text-md">
              ...
              <div className="relative w-full col-span-12 text-center">
                <button
                  onClick={resetStates}
                  className="text-base sm:text-sm md:text-md lg:text-base xl:text-2xl  2xl:text-md capitalize bg-white  w-80 h-20  mb-3 text-gray-500 ml-2 font-bold py-2 px-4 border border-gray-900 rounded"
                >
                  <FormattedMessage id="Reset" defaultMessage="Reset" />
                </button>
                &nbsp;&nbsp;&nbsp;&nbsp;
                <button
                  className={`text-base sm:text-sm md:text-md lg:text-base xl:text-2xl  2xl:text-md capitalize bg-main-blue hover:bg-main-blue mb-3 w-80 h-20 text-white ml-2 font-bold py-2 px-4 border border-blue-700 rounded ${
                    disableUploadButton ? 'bg-opacity-10' : ''
                  }`}
...
function SampleDataTable() {
  ...
  const newdata = {
    ...
    downloads: [
      '',
      `${config.media}samples/clinicalinformation.tsv`,
      `${config.media}samples/dna_mutation.tsv`,
      `${config.media}samples/rna.tsv`,
      `${config.media}samples/methylation.tsv`,
      `${config.media}samples/phospho.tsv`,
      `${config.media}samples/proteome.tsv`,
      `${config.media}samples/fusion.tsv`,
      `${config.media}samples/cnv.tsv`
    ],
    body: [
    ...
    componets: [
      CircosPlotGuidelines,
      OncoprintGuidelines,
      LollipopGuidelines,
      VolcanoPlotGuidelines,
      HeatmapGuideliens,
      SurvivalGuidelines,
      FusionPlotGuidelines,
      CNVGuidelines,
      BoxPlotGuidelines,
      CorrelationGuidelines
    ]
  };
  return (
                    ...
                    if (i !== 0) {
                      return (
                        <td
                          ...
                          className="px-6 py-4 whitespace-nowrap text-center text-lg font-medium border"
                        >
                      ...
                      {row.map((cellData, cellIndex) => {
                        if (cellIndex === 0) {
                          ...
                          return (
                            <td
                              trindex={index}
                              key={`${index}-${cellIndex}-${cellData}` + Math.random() * 300}
                              className="px-6 py-4 whitespace-nowrap border"
                            >
                              <div
                                trindex={index}
                                className="capitalize text-lg text-gray-900"
                                onClick={(e) => {
                                  for (i = 0; i <= 9; i++) {
                                    if (i !== index) {
                          ...
                          if (cellData === true) {
                            return (
                              <td
                                key={`${index}-${cellIndex}-${cellData}` + Math.random() * 100}
                                className="px-6 py-4 whitespace-nowrap text-center border"
                              >
                ...
                <div className="flex items-start justify-between p-5 border-b border-solid border-blueGray-200 rounded-t">
                  <h6 className="font-semibold">
                    <FormattedMessage
                      ...
                      defaultMessage="File Type Description"
                    />
                  </h6>
                </div>
                {/*body*/}
