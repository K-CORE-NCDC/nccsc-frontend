          ...
          <li
            ...
            className={
              children[j]['id'] === 'otherservice'
                ? 'child-group h-full relative px-3 py-4'
                : 'px-3 py-5'
            }
          >
            <Link
              ...
              className={`text-3xl py-2 px-4 block whitespace-no-wrap ${
                children[j]['id'] === 'otherservice' ? 'relative hover-trigger' : ''
              }`}
              to={children[j].url}
            >
              {children[j].title}
            ...
            {children[j]['id'] === 'otherservice' && (
              <ul
                className="list-none text-center h-auto absolute hidden hover-target"
                style={{ left: '100%', top: '5%', height: '125px', backgroundColor: ' #0c3c6a' }}
              >
                <li key={i + '_' + j} className="px-3 py-5">
                  <Link
                    key={i + 'link_' + j}
                    className="text-3xl py-2 px-4  whitespace-no-wrap"
                    to="/organoid"
                  >
      ...
      if (tmp.length > 0) {
        ...
        html.push(
          <li className="group h-full table-cell relative " key={'liul_' + i}>
            {htmlbutton}
            <ul className="absolute hidden list-none text-center h-auto" key={'ul_' + i}></ul>
          </li>
        );
      }
    }
